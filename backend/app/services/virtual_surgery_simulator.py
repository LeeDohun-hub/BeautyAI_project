...
CONSULT_TIERS: tuple[tuple[str, str], ...] = (
    # ⚠ 1단계는 **의사 상담이 아니다.** 목록 제목이 '의사 상담 시술 후보' 이므로
    #   여기만 '시술 없이' 로 못박아 두지 않으면 메이크업이 시술처럼 읽힌다.
    ("makeup", "시술 없이 — 메이크업·스타일링"),
    ("derma", "피부과·쁘띠 시술"),
    ("plastic", "성형외과 상담"),
    ("surgery", "수술적 접근"),
)
...
CONSULT_CANDIDATE_NOTE = (
    # ⚠ 마크다운 강조(**)를 쓰지 말 것. 화면은 이 문자열을 **평문으로 그대로** 출력한다
    #   (JSX 는 마크다운을 해석하지 않는다). 실제로 '**후보**' 가 별표째 노출됐다.
    "아래는 상담에서 이야기해 볼 수 있는 후보이며, 필요한 시술을 정한 것이 아닙니다. "
    "실제 가능 여부와 방법은 의료진이 판단합니다."
)
# 4분류를 왜 나누는지 화면에서도 말해 준다(설계안 §9의 의도).
CONSULT_TIER_NOTE = (
    "가벼운 방법부터 순서대로 정리했습니다. 위쪽만으로 충분한 경우도 많습니다. "
    "시술 항목은 의사와 상담할 때 이야기해 볼 후보입니다."
)
