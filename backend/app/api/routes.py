...
_NON_COSMETIC_RE = re.compile(
    ...
    #   '인솔·뒤꿈치'는 국내몰 남성 주입을 켜면서 필요해졌다 — '닥터숄 발뒤꿈치 쿠션 기능성
    #   인솔 남자용'과 '닥터숄 뒤꿈치 전용 쿠션 남자용'이 '쿠션'+'남자'로 잡혀 베이스 컬럼에
    #   카드로 떴다(실측 2026-08-07). '뒤꿈치'가 걸리는 나머지는 풋크림·풋필링인데, 어차피
    #   색조 컬럼에 들어갈 물건이 아니라 배제가 맞다.
    #   ⚠ '발보호'는 넣지 않는다 — '모발보호'(헤어) 안에 들어 있다(이 저장소에서 반복된
    #     부분문자열 오탐 부류: nail⊂snail, 아이⊂아이보리, 러그⊂컬러그램).
    r"|눈썹칼|눈썹가위|눈썹\s*정리|쉐이버|트리머|면도기|면도날|眉毛シェーバー|カミソリ|인솔|중창|뒤꿈치"
